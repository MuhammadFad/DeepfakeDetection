...
def _generate_accuracy_chart(summary_stats: dict) -> str:
    ...
    fig.update_layout(
        ...
        xaxis=dict(gridcolor='rgba(255,255,255,0.06)', tickfont=dict(size=13, color='#c9d1d9'), showline=False),
        yaxis=dict(range=[0, 115], gridcolor='rgba(255,255,255,0.06)', tickfont=dict(size=12, color='#8b949e'),
                   title=dict(text='Accuracy (%)', font=dict(size=13)), showline=False, zeroline=False),
        height=360, margin=dict(t=24, b=16, l=56, r=24), bargap=0.35,
    )
...
def _generate_training_curves(history: dict) -> str:
    ...
    for mn in MODEL_ORDER:
        ...
        solid, _ = COLORS.get(mn, ('#aaa', '#666'))
        name = MODEL_DISPLAY_NAMES.get(mn, mn)
        fig.add_trace(go.Scatter(
            x=epochs, y=h['train_acc'], mode='lines+markers',
            name=f'{name} train',
            line=dict(color=solid, width=2, dash='dot'), marker=dict(size=5),
            hovertemplate='Epoch %{x}<br>Train acc: %{y:.1f}%<extra>' + name + '</extra>',
        ))
        if any(v > 0 for v in h.get('val_acc', [])):
            fig.add_trace(go.Scatter(
                x=epochs, y=h['val_acc'], mode='lines+markers',
                name=f'{name} val',
                line=dict(color=solid, width=2.5), marker=dict(size=6, symbol='diamond'),
                hovertemplate='Epoch %{x}<br>Val acc: %{y:.1f}%<extra>' + name + ' val</extra>',
            ))
    fig.update_layout(
        paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
        font=dict(color='#8b949e', family='Segoe UI, system-ui, sans-serif'),
        xaxis=dict(title='Epoch', gridcolor='rgba(255,255,255,0.06)', tickfont=dict(size=12, color='#c9d1d9')),
        yaxis=dict(title='Accuracy (%)', range=[0, 105], gridcolor='rgba(255,255,255,0.06)',
        ...
        height=380, margin=dict(t=16, b=40, l=56, r=24), hovermode='x unified',
    )
...
def generate_html_report(image_results: list, summary_stats: dict) -> str:
    ...
    for row in sample_rows:
        ...
        for mn in MODEL_ORDER:
            ...
            if label == 'N/A':
                cell_cls, conf_str = 'td-na', '—'
            elif label == gt:
                cell_cls, conf_str = 'td-correct', f'{conf:.1f}%'
            else:
                cell_cls, conf_str = 'td-wrong', f'{conf:.1f}%'
  ...
  <section class="section">
    ...
    <div class="chart-box">{curves_div}</div>
    ...
    model_th     = ''.join(f'<th colspan="2" class="th-model">{MODEL_DISPLAY_NAMES.get(mn, mn)}</th>' for mn in MODEL_ORDER)
...
<style>
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  body {{ background: #0d1117; color: #e6edf3; font-family: 'Segoe UI', system-ui, -apple-system, sans-serif; line-height: 1.6; min-height: 100vh; }}
  .header {{ background: linear-gradient(160deg, #161b22 0%, #0d1117 100%); border-bottom: 1px solid #21262d; padding: 48px 32px 40px; text-align: center; }}
  .header-icon {{ font-size: 2.8rem; margin-bottom: 12px; display: block; }}
  .header h1 {{ font-size: 2.4rem; font-weight: 700; background: linear-gradient(90deg, #58a6ff 0%, #79c0ff 50%, #a5f3fc 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; letter-spacing: -0.5px; margin-bottom: 6px; }}
  .header .subtitle {{ color: #8b949e; font-size: 1.05rem; }}
  .header .timestamp {{ color: #484f58; font-size: 0.82rem; margin-top: 6px; letter-spacing: 0.03em; }}
  .page {{ max-width: 1440px; margin: 0 auto; padding: 32px 24px 64px; }}
  .section {{ margin-bottom: 48px; }}
  .section-title {{ font-size: 1.05rem; font-weight: 600; color: #58a6ff; border-left: 3px solid #1f6feb; padding-left: 12px; margin-bottom: 20px; text-transform: uppercase; letter-spacing: 0.06em; }}
  .cards-row {{ display: flex; gap: 16px; flex-wrap: wrap; }}
  .card {{ flex: 1; min-width: 180px; background: #161b22; border: 1px solid #30363d; border-radius: 12px; padding: 24px 20px; text-align: center; transition: border-color 0.2s, transform 0.15s; }}
  .card:hover {{ border-color: #58a6ff; transform: translateY(-2px); }}
  .card-label {{ color: #6e7681; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.07em; margin-bottom: 6px; }}
  .card-value {{ font-size: 2.6rem; font-weight: 700; line-height: 1.1; margin-bottom: 4px; }}
  .card-sub {{ color: #6e7681; font-size: 0.8rem; }}
  .chart-box {{ background: #161b22; border: 1px solid #30363d; border-radius: 12px; padding: 20px 12px 8px; overflow: hidden; }}
  .table-scroll {{ overflow-x: auto; border-radius: 12px; border: 1px solid #30363d; }}
  table {{ width: 100%; border-collapse: collapse; font-size: 0.88rem; background: #161b22; white-space: nowrap; }}
  thead tr:first-child {{ background: #0d1117; }} thead tr:last-child {{ background: #10161d; }}
  th {{ padding: 11px 14px; color: #6e7681; font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; border-bottom: 1px solid #30363d; text-align: center; }}
  .th-model {{ color: #79c0ff; font-size: 0.8rem; border-left: 1px solid #21262d; }}
  td {{ padding: 9px 14px; border-bottom: 1px solid #21262d; text-align: center; vertical-align: middle; }}
  tr:last-child td {{ border-bottom: none; }} tr:nth-child(even) {{ background: #0d1117; }} tr:hover {{ background: #161b22 !important; }}
  .td-name {{ text-align: left; color: #c9d1d9; font-family: 'Cascadia Code', 'Courier New', monospace; font-size: 0.8rem; max-width: 180px; overflow: hidden; text-overflow: ellipsis; }}
  .td-correct {{ color: #3fb950; font-weight: 600; }} .td-wrong {{ color: #f85149; font-weight: 600; }} .td-na {{ color: #484f58; }} .td-conf {{ color: #6e7681; font-size: 0.82rem; }}
  .badge-real {{ background: rgba(63,185,80,0.15); color: #3fb950; border: 1px solid rgba(63,185,80,0.3); border-radius: 6px; padding: 2px 8px; font-size: 0.78rem; font-weight: 600; }}
  .badge-fake {{ background: rgba(248,81,73,0.12); color: #f85149; border: 1px solid rgba(248,81,73,0.3); border-radius: 6px; padding: 2px 8px; font-size: 0.78rem; font-weight: 600; }}
  .cm-row {{ display: flex; gap: 20px; flex-wrap: wrap; }}
  .cm-card {{ flex: 1; min-width: 220px; background: #161b22; border: 1px solid #30363d; border-radius: 12px; padding: 20px; }}
  .cm-title {{ color: #79c0ff; font-size: 0.88rem; font-weight: 600; text-align: center; margin-bottom: 14px; }}
  .cm-table {{ width: 100%; border-collapse: collapse; font-size: 0.82rem; }}
  .cm-table th, .cm-table td {{ padding: 9px 12px; text-align: center; border: none; background: transparent; }}
  .cm-corner {{ width: 30%; }} .cm-head {{ color: #6e7681; font-size: 0.72rem; text-transform: uppercase; letter-spacing: 0.04em; }}
  .cm-label {{ color: #6e7681; font-size: 0.72rem; text-transform: uppercase; letter-spacing: 0.04em; text-align: left; }}
  .cm-ok {{ background: rgba(63,185,80,0.12) !important; color: #3fb950; border-radius: 8px; font-size: 1.3rem; font-weight: 700; }}
  .cm-bad {{ background: rgba(248,81,73,0.10) !important; color: #f85149; border-radius: 8px; font-size: 1.3rem; font-weight: 700; }}
  .cm-badge {{ display: block; font-size: 0.62rem; font-weight: 400; color: #484f58; margin-top: 2px; text-transform: uppercase; letter-spacing: 0.05em; }}
  .interp-box {{ background: #161b22; border: 1px solid #30363d; border-left: 3px solid #1f6feb; border-radius: 12px; padding: 24px 28px; color: #c9d1d9; line-height: 1.85; font-size: 0.95rem; }}
  .interp-box strong {{ color: #79c0ff; }}
  .legend {{ display: flex; gap: 20px; flex-wrap: wrap; margin-bottom: 14px; font-size: 0.82rem; color: #8b949e; }}
  .legend-dot {{ display: inline-block; width: 10px; height: 10px; border-radius: 50%; margin-right: 5px; vertical-align: middle; }}
  .gallery-grid {{ display: grid; grid-template-columns: repeat(auto-fill, minmax(340px, 1fr)); gap: 16px; }}
  .gallery-card {{ background: #161b22; border: 1px solid #30363d; border-radius: 12px; padding: 16px; border-top-width: 3px; }}
  .gc-correct {{ border-top-color: #3fb950; }} .gc-wrong {{ border-top-color: #f85149; }} .gc-mixed {{ border-top-color: #d29922; }} .gc-neutral {{ border-top-color: #30363d; }}
  .gc-header {{ display: flex; align-items: center; justify-content: space-between; gap: 8px; margin-bottom: 12px; }}
  .gc-name {{ color: #8b949e; font-family: 'Cascadia Code', 'Courier New', monospace; font-size: 0.75rem; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }}
  .gc-images {{ display: flex; gap: 6px; }} .gc-fig {{ flex: 1; min-width: 0; text-align: center; margin: 0; }}
  .gc-img-box {{ width: 100%; aspect-ratio: 1; background: #0d1117; border-radius: 6px; overflow: hidden; display: flex; align-items: center; justify-content: center; }}
  .gc-thumb {{ width: 100%; height: 100%; object-fit: cover; display: block; }} .gc-no-img {{ color: #484f58; font-size: 0.65rem; }}
  .gc-fig figcaption {{ margin-top: 5px; color: #6e7681; font-size: 0.68rem; line-height: 1.4; }}
  .pred-correct {{ color: #3fb950; font-weight: 600; }} .pred-wrong {{ color: #f85149; font-weight: 600; }} .pred-na {{ color: #484f58; }}
  .cam-note {{ color: #6e7681; font-size: 0.82rem; margin-bottom: 16px; padding-left: 4px; }}
  footer {{ text-align: center; padding: 28px; color: #484f58; font-size: 0.8rem; border-top: 1px solid #21262d; }}
...
<main class="page">
  <section class="section">
    ...
    <div class="cards-row">{total_card}{model_cards}</div>
  </section>
  <section class="section">
  ...
  <section class="section">
    ...
    <div class="table-scroll">
      <table>
        <thead>
          <tr><th rowspan="2" style="text-align:left">Image</th><th rowspan="2">Ground Truth</th>{model_th}</tr>
          <tr>{model_sub_th}</tr>
        </thead>
...
<footer>Generated by <span>Deepfake Detection System</span> &bull; {timestamp}</footer>
</body>
</html>"""
